...
include device/namtso/common/build/rockchip/F2fsCompression.mk
endif

include device/namtso/common/build/rockchip/Partitions.mk
...
PRODUCT_PARAMETER_TEMPLATE ?= device/namtso/common/scripts/parameter_tools/parameter.in
...
PRODUCT_FSTAB_TEMPLATE ?= device/namtso/common/scripts/fstab_tools/fstab_go.in
else
PRODUCT_FSTAB_TEMPLATE ?= device/namtso/common/scripts/fstab_tools/fstab.in
endif

# default.prop & build.prop split
BOARD_PROPERTY_OVERRIDES_SPLIT_ENABLED ?= true

DEVICE_MANIFEST_FILE ?= device/namtso/common/manifests/manifest_level_$(ROCKCHIP_LUNCHING_API_LEVEL).xml
ifeq (1,$(strip $(shell expr $(ROCKCHIP_LUNCHING_API_LEVEL) \>= 31)))
# Android S deprecate schedulerservice, use ioprio in init.rc
DEVICE_MATRIX_FILE   ?= device/namtso/common/manifests/compatibility_matrix_level_31.xml
else
# For Android R and older versions.
DEVICE_MATRIX_FILE   ?= device/namtso/common/manifests/compatibility_matrix.xml
endif
...
DEVICE_PACKAGE_OVERLAYS += device/namtso/common/overlay
ifneq ($(BOARD_HAS_RK_4G_MODEM), true)
DEVICE_PACKAGE_OVERLAYS += device/namtso/common/overlay_wifi_only
endif
endif

TARGET_RELEASETOOLS_EXTENSIONS := device/namtso/common
...
# Wifi&Bluetooth
include device/namtso/common/wifi_bt_common.mk
...
# gralloc 4.0
include device/namtso/common/gralloc.device.mk
...
ROCKCHIP_OEM_CONFIG_FILE ?= device/namtso/common/configs/cfg_rockchip_default.xml
ROCKCHIP_OEM_CONFIG_PACKAGES ?= device/namtso/common/configs/rockchip_forbid_packages.xml
...
#    TARGET_BASE_PARAMETER_IMAGE ?= device/namtso/common/baseparameter/baseparameter_fb720.img
...
PRODUCT_DEFAULT_DEV_CERTIFICATE := device/namtso/common/security/testkey
...
ifeq ($(strip $(BOARD_BASEPARAMETER_SUPPORT)), true)
    ifneq ($(filter rk356x rk3588, $(strip $(TARGET_BOARD_PLATFORM))), )
        TARGET_BASE_PARAMETER_IMAGE ?= device/namtso/common/baseparameter/v2.0/baseparameter.img
    else
        TARGET_BASE_PARAMETER_IMAGE ?= device/namtso/common/baseparameter/v1.0/baseparameter.img
    endif
