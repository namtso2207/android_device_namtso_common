...
TARGET_SYSTEM_PROP += device/namtso/common/build/rockchip/rksdk.prop
...
# Prebuild apps
$(call inherit-product, device/namtso/common/modules/preinstall.mk)
$(call inherit-product, device/namtso/common/modules/optimize.mk)
$(call inherit-product, device/namtso/common/modules/build_dm.mk)
...
ifeq ($(strip $(TARGET_BOARD_PLATFORM_PRODUCT)), atv)
  ...
  $(call inherit-product, device/namtso/common/tv/tv_base.mk)
...
# PCBA tools
$(call inherit-product, device/namtso/common/modules/pcba.mk)
# Optee
$(call inherit-product, device/namtso/common/modules/optee.mk)
# Sepolicy
$(call inherit-product, device/namtso/common/modules/android_sepolicy.mk)
# TWRP
$(call inherit-product, device/namtso/common/modules/twrp.mk)
# GMS
$(call inherit-product, device/namtso/common/modules/gms.mk)
# Media OMX/C2
$(call inherit-product, device/namtso/common/modules/mediacodec.mk)
# Android Go configuration
$(call inherit-product, device/namtso/common/modules/android_go.mk)
# Android Verified Boot
$(call inherit-product, device/namtso/common/modules/avb.mk)
# init.rc files
$(call inherit-product, device/namtso/common/rootdir/rootdir.mk)
ifeq ($(strip $(BOARD_HDMI_IN_SUPPORT)), true)
    $(call inherit-product, device/namtso/common/modules/hdmi_in.mk)
endif
...
PRODUCT_COPY_FILES += \
    device/namtso/common/rk29-keypad.kl:system/usr/keylayout/rk29-keypad.kl \
    device/namtso/common/ff680030_pwm.kl:system/usr/keylayout/ff680030_pwm.kl \
    device/namtso/common/alarm_filter.xml:system/etc/alarm_filter.xml \
    device/namtso/common/ff420030_pwm.kl:system/usr/keylayout/ff420030_pwm.kl
...
$(call inherit-product, device/namtso/common/modules/4g_modem.mk)
endif
...
# Then include the device/namtso/common/build/rockchip/RebuildFstab.mk in your AndroidBoard.mk
PRODUCT_COPY_FILES += \
...
# Bluetooth
$(call inherit-product, device/namtso/common/modules/bluetooth.mk)
...
$(call inherit-product, device/namtso/common/modules/thermal.mk)

# include vibrator AIDL module
$(call inherit-product, device/namtso/common/modules/vibrator.mk)

# Media DRM
$(call inherit-product, device/namtso/common/modules/media_drm.mk)

# Usb controller detector for GKI
$(call inherit-product, device/namtso/common/modules/usb.mk)

# GKI modules
$(call inherit-product, device/namtso/common/modules/gki_common.mk)

# kernel configurations
$(call inherit-product, device/namtso/common/modules/kernel_config.mk)

# make boot/vendor_boot
$(call inherit-product, device/namtso/common/modules/make_boot.mk)

# recovery
$(call inherit-product, device/namtso/common/modules/recovery.mk)
...
DEVICE_MANIFEST_FILE += \
    device/namtso/common/manifests/android.hardware.graphics.mapper@4.0.xml
else # Use HIDL
...
DEVICE_MANIFEST_FILE += \
    device/namtso/common/manifests/android.hardware.graphics.mapper@4.0.xml \
    device/namtso/common/manifests/android.hardware.graphics.allocator@4.0.xml
else
PRODUCT_PROPERTY_OVERRIDES += \
    ro.vendor.mpp_buf_type=1
PRODUCT_PACKAGES += \
...
DEVICE_MANIFEST_FILE += \
    device/namtso/common/manifests/android.hardware.graphics.mapper@2.1.xml \
    device/namtso/common/manifests/android.hardware.graphics.allocator@2.0.xml
endif
endif
...
$(call inherit-product, device/namtso/common/modules/health.mk)
...
ifeq ($(strip $(TARGET_BOARD_PLATFORM_PRODUCT)), box)
    #include device/namtso/common/samba/rk31_samba.mk
    PRODUCT_COPY_FILES += \
      $(LOCAL_PATH)/init.box.samba.rc:$(TARGET_COPY_OUT_VENDOR)/etc/init/hw/init.box.samba.rc \
      device/namtso/common/cifsmanager.sh:system/bin/cifsmanager.sh
...
ifeq ($(BOARD_SUPPORT_HDMI_CEC),true)
  $(call inherit-product, device/namtso/common/modules/hdmi_cec.mk)
endif
...
PRODUCT_COPY_FILES += \
    device/namtso/common/permissions/rockchip.software.display.xml:system/etc/permissions/rockchip.software.display.xml
endif
...
ifeq ($(strip $(TARGET_BOARD_PLATFORM_PRODUCT)), vr)
PRODUCT_COPY_FILES += \
       device/namtso/common/lowmem_package_filter.xml:system/etc/lowmem_package_filter.xml 
endif
...
include device/namtso/common/bootshutdown/bootshutdown.mk
PRODUCT_PROPERTY_OVERRIDES += \
...
include device/namtso/common/bootvideo/bootvideo.mk
endif
...
ifeq ($(TARGET_BOARD_PLATFORM_PRODUCT),box)
  DEVICE_PACKAGE_OVERLAYS += device/namtso/common/overlay_screenoff

  $(call inherit-product, device/namtso/common/modules/rockchip_apps_box.mk)
...
$(call inherit-product, device/namtso/common/modules/rockchip_apps.mk)
...
DEVICE_PACKAGE_OVERLAYS += device/namtso/common/overlay_brightness
endif
...
PRODUCT_COPY_FILES += \
    device/namtso/common/uimode/package_uimode_config.xml:vendor/etc/package_uimode_config.xml

# Zoom out recovery ui of box by two percent.
ifneq ($(filter atv box, $(strip $(TARGET_BOARD_PLATFORM_PRODUCT))), )
    TARGET_RECOVERY_OVERSCAN_PERCENT := 2
    TARGET_BASE_PARAMETER_IMAGE ?= device/namtso/common/baseparameter/baseparameter.img
    # savBaseParameter tool
    ifneq (,$(filter userdebug eng, $(TARGET_BUILD_VARIANT)))
        PRODUCT_PACKAGES += saveBaseParameter
    endif
    DEVICE_FRAMEWORK_MANIFEST_FILE := device/namtso/common/manifest_framework_override.xml
endif
...
PRODUCT_COPY_FILES += \
    device/namtso/common/flash_img/flash_img.sh:vendor/bin/flash_img.sh

#read pcie info for Devicetest APK
PRODUCT_COPY_FILES += \
    device/namtso/common/pcie/read_pcie_info.sh:vendor/bin/read_pcie_info.sh

BOARD_TV_LOW_MEMOPT ?= false
ifeq ($(strip $(BOARD_TV_LOW_MEMOPT)), true)
    include device/namtso/common/tv/tv_low_ram_device.mk
endif

# Camera support
ifeq ($(BOARD_CAMERA_SUPPORT),true)
$(call inherit-product, device/namtso/common/modules/camera.mk)
endif

# Rockchip HALs
$(call inherit-product, device/namtso/common/manifests/frameworks/vintf.mk)

ifeq ($(BOARD_MEMTRACK_SUPPORT),true)
$(call inherit-product, device/namtso/common/modules/memtrack.mk)
endif
